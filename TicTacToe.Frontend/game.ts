...
function updateBoard(gamestate: any): void{
    if(gamestate.isOver === false){status.textContent = "Player turn: "+gamestate.currentPlayer;}
    let counter = 0;
    cells.forEach(cell => {
    ...
    if(gamestate.winner !== null){winner.textContent="Winner is "+gamestate.winner;status.textContent="Game Over";gameComplete=true;}
    else if (gamestate.winner ===null && gamestate.isOver){winner.textContent="It is a draw"; status.textContent="Game Over";gameComplete=true;}
    else winner.textContent = null;
}